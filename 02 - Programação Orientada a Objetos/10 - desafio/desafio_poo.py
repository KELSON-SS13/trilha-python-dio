...
class Conta:
    ...
    Atributos:
        ...
        cliente (Cliente): O objeto cliente associado à conta.
        historico (Historico): O histórico de transações da conta.
    """
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        ...
        transacoes_saque = self.historico.transacoes_do_dia(tipo_transacao="Saque")
        
        excedeu_limite = valor > self.limite
        excedeu_saques = len(transacoes_saque) >= self.limite_saques
...
class Historico:
    ...
    def transacoes_do_dia(self, tipo_transacao=None):
    ...
    def gerar_relatorio(self, tipo_transacao=None):
        """Gera um relatório de transações, com opção de filtro."""
        for transacao in self._transacoes:
            if tipo_transacao is None or transacao["tipo"].lower() == tipo_transacao.lower():
                yield transacao

class Transacao(ABC):
    """Interface para as transações, implementada como uma Classe Abstrata."""
    @property
    @abstractmethod
    def valor(self):
        pass
...
class Saque(Transacao):
    """Classe para a transação de Saque."""
...
class Deposito(Transacao):
    """Classe para a transação de Depósito."""
    def __init__(self, valor):
        self._valor = valor
...
# --- Funções Decoradoras e de Interface ---
...
@log_transacao
def operacao_depositar(clientes):
...
@log_transacao
def operacao_exibir_extrato(clientes):
    cpf = input("Informe o CPF do cliente: ")
    cliente = filtrar_cliente(cpf, clientes)
...
@log_transacao
def operacao_criar_cliente(clientes):
...
@log_transacao
def operacao_criar_conta(numero_conta, clientes, contas):
    cpf = input("Informe o CPF do cliente: ")
    cliente = filtrar_cliente(cpf, clientes)
...
def main():
    ...
    while True:
        ...
        if opcao == "d":
            operacao_depositar(clientes)
        
        elif opcao == "s":
            operacao_sacar(clientes)

        elif opcao == "e":
            operacao_exibir_extrato(clientes)
            
        elif opcao == "nu":
            operacao_criar_cliente(clientes)

        elif opcao == "nc":
            numero_conta = len(contas) + 1
            operacao_criar_conta(numero_conta, clientes, contas)
